...
Three trap-score files exist in this repository and they are not interchangeable. This document says exactly how they differ and which one the paper uses.

| File | Status |
|---|---|
| `data/scores_traps_uncorrected.csv` | output of the earlier scorer version — kept only so the correction is auditable. **Do not use for analysis.** Never overwritten. |
| `data/scores_traps_fixed.csv` | corrected — **this is what the paper reports.** Never overwritten. |
| `data/scores_traps.csv` | scratch output of the most recent `score_all.py` run. Overwritten every time `reproduce.sh` runs. Because the shipped `scorer.py` already contains the fix, a fresh run reproduces `scores_traps_fixed.csv` exactly. |

`reproduce.sh` step 2 checks both facts: the uncorrected file differs from the fixed file in exactly 4 rows, and a fresh re-score matches the fixed file in all 1,215.
