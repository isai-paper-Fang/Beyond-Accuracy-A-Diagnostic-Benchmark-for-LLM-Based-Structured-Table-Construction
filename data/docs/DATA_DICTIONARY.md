...
Every `task` column in every file uses **data-file test IDs**. See [`TEST_ID_MAPPING.md`](TEST_ID_MAPPING.md).
...
## `data/raw/R###_<Model>_<Task>_rep<N>.txt` — 72 files

The verbatim response of each run. 54 belong to the three systems in the paper; 18 are from a fourth system that was excluded (see the paper's Limitations). Nothing has been edited — the files include the free-text reasoning that preceded the JSON block.
